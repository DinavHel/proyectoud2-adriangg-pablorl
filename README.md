# proyectoud2-adriangg-pablorl
## 1. Idea base del proyecto
En base al proyecto realizado en la Unidad 1, modificaremos el proyecto para que realice
una conexión una base de datos alojada en la máquina local.
## 2. Manual técnico para desarrolladores

## 3. Manual de usuario

## 4. Reparto de las tareas entre ambos integrantes
* **Pablo Rodríguez Lomba**
    + X
* **Adrián González Gómez**
    + X
## 5. Extras realizados


## 6. Propuestas de mejora

## 7. Conclusiones y opinión del trabajo realizado
